# Odyssey Voyage II - Server (Airlock) — Final Version
...
To get started (from the `final/monolith` directory):
...
Next, let's run some local services. In a new terminal window, run:
...
1. Run `pnpm install` (or `npm install`) in the `final` directory.
1. Run `pnpm start` (or `npm start`) in the `final` directory.

This will start the GraphQL API gateway on [http://localhost:4000](http://localhost:4000).

Next, let's run the subgraphs: the monolith subgraph, the accounts subgraph, and the listings subgraph.

1. In a new terminal, navigate to `final` and run `pnpm run start:monolith-subgraph` (or `npm run start:monolith-subgraph`).
1. In a new terminal, navigate to `final/subgraph-accounts` and run `pnpm install && pnpm start` (or `npm install && npm start`).

Finally, in a new terminal in the `final` directory, run:
