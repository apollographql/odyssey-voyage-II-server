# Start-Apollo.ps1
# Starts all Apollo Voyage II services in the correct order
# Run from: F:\DEV\apollo\odyssey-voyage-II-server
# Usage: .\Start-Apollo.ps1
...
Write-Host "[1/6] Starting REST services (ports 4010, 4011)..." -ForegroundColor Yellow
Start-ServiceWindow -Title "REST Services (pnpm launch)" `
  -WorkingDir "$RootServer\monolith" `
  -Command "pnpm launch"

Write-Host "      Waiting 4s for REST services to initialize..." -ForegroundColor Gray
Start-Sleep -Seconds 4

# Step 2: Monolith subgraph (port 4001)
Write-Host "[2/6] Starting monolith subgraph (port 4001)..." -ForegroundColor Yellow
Start-ServiceWindow -Title "Monolith (4001)" `
  -WorkingDir "$RootServer\monolith" `
  -Command "pnpm start"

Start-Sleep -Seconds 2

# Step 3: Accounts subgraph (port 4002)
Write-Host "[3/6] Starting subgraph-accounts (port 4002)..." -ForegroundColor Yellow
...
# Step 4: Listings subgraph (port 4003)
Write-Host "[4/6] Starting subgraph-listings (port 4003)..." -ForegroundColor Yellow
...
# Step 5: rover dev - Router (port 4000)
Write-Host "[5/6] Starting rover dev / Router (port 4000)..." -ForegroundColor Yellow
...
# Step 6: Client (port 3000)
Write-Host "[6/6] Starting client (port 3000)..." -ForegroundColor Yellow
...
Write-Host "  Monolith      : http://localhost:4001" -ForegroundColor White
